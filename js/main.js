...
//       for(let i = 0; i < 401; i++){
//         // if(response[i].zipcode != response[i+1].zipcode){
...
//         let listItem = document.querySelector("#centerName").textContent = `The  ${response[i].center}  is located at zipcode: ${response[i].zipcode} `  //each of these inputs, takes the text value and puts it into the new li;check whats actually written not just the box itself
...
//
//         const zipCode = response[i].zipcode
//         fetch()
//         .then(res => res.json()) // parse response as JSON (can be res.text() for plain response)
//         .then(weather => {
//           console.log(weather)
...
.then(response => {
  response.forEach( (location, i) => {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?zip=${zip},us&appid=112b7296f18313c8a2da4652f6171d42`)
    .then(res => res.json())
    .then(weather => {
